...
def reflectance(geometry, wave, materials, n_mod):  
    ...
    return Rup#, Rdown #Tup, Tdown

### Swag-structure : 
# Ag / dielec variable perm (3 nm) / Au (5nm) / ITO (100nm) / Glass (for comparison with Beniamino's files, PM)
thick_super = 200
...
thick_func = 0 # présent partout tout le temps
thick_mol = 0 # si molécules détectées
thick_metal = 5 # hauteur de l'or au dessus du substrat
thick_accroche = 100 # couche d'accroche 
period = 100.2153
#period = 300.2153
#period = 500.2153
thick_sub = 200
...
list_perm_dielec = np.linspace(1.4, 2, 5)**2
...
R = np.empty((list_perm_dielec.size, list_wavelength.size))
...
plt.figure(3)
for idx_gap, perm_dielec in enumerate(list_perm_dielec):
    plt.plot(list_wavelength, R[idx_gap,:], label = f"Gap permittivity : {perm_dielec}")
    plt.legend()
