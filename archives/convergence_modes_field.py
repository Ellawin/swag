...
# Cubes
Largeur_cube = 71.1 # largeur du cube
Hauteur_dielec = 5 # hauteur de diéléctrique en dessous du cube
hauteur_cube = 75 #hauteur du cube
Hauteur_gold = 0 # hauteur de l'or au dessus du substrat
period = 501.2153 # periode
...
n_mod_list = np.linspace(3,150,150-3)
max_M = np.empty(n_mod_list.size)

#n_mod = 50 # inf à 50 sinon différences avec Octave
for n_mod in n_mod_list:
    n_mod = int(n_mod)
...
# Matrices descendantes
    ...
    max_M[idx] = Mfield.max()
    idx += 1

    plt.figure(n_mod)
    plt.title("Field map for %d nmod" %n_mod)
    plt.imshow(Mfield, cmap = 'jet', aspect = 'auto')
    plt.colorbar()
#    plt.show(block=False)
    plt.savefig("fig/convergence_modes/field_%d_nmod.pdf" %n_mod)


plt.figure(300)
plt.plot(n_mod_list, max_M)
plt.title("Max de M convergence modes")
plt.show(block=False)
plt.savefig("fig/convergence_modes/MaxMconvergenceModes")