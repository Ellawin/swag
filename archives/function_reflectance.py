...
def reflectance(L, H, h, wavelength, period, polarization, perm_dielec, perm_metal, n):
...
n_mod = 50 # to do convergence study
n = 2 * n_mod + 1
...
H = 75 # taille du résonateur selon z, en nm. Cube au départ donc H = L
h = 5 # taille du gap dans lequel résone le GP, dans la couche homogène de dielec. Equivalent à 'spacer'

for L in list_L:   
    R[idx] = reflectance(L, H, h, wavelength, period, polarization, perm_dielec, perm_metal, n)
    idx += 1

    cavity_size2 = L / period # taille du résonateur selon x, en nm
    H2 = 75 / period # taille du résonateur selon z, en nm. Cube au départ donc H = L
    h2 = 5 / period # taille du gap dans lequel résone le GP, dans la couche homogène de dielec. Equivalent à 'spacer'
    #x0 = [(1 - cavity_size )/ 2] # position du bloc dans la période. Central. 
    x02 = 0.5 - cavity_size2 / 2

    wavelength_norm2 = wavelength / period 
    k02 = 2 * np.pi / wavelength_norm2
    P2, V2 = homogene(k02, 0, polarization, perm_dielec, n)
    S2 = np.block([[np.zeros([n, n]), np.eye(n, dtype=np.complex128)], [np.eye(n), np.zeros([n, n])]])
    Pc2, Vc2 = creneau(k02, 0, polarization, perm_metal, perm_dielec, cavity_size2, n, x02)
    S2 = cascade(S2, interface(P2, Pc2))
    S2 = c_bas(S2, Vc2, H2)
    S2 = cascade(S2, interface(Pc2, P2))
    S2 = c_bas(S2, V2, h2)
    Pc2, Vc2 = homogene(k02, 0, polarization, perm_metal, n)
    S2 = cascade(S2, interface(P2, Pc2))
    R_creneau[idx_c] = abs(S2[n_mod, n_mod]) ** 2 * np.real(V2[n_mod]) / k02
    idx_c += 1


plt.figure(1)
#plt.title("wavelength dependance")
#plt.plot(list_L, R_reseau, "xr",  label="reseau")
plt.plot(list_L, R_creneau, "og", label = "sans fonction")
plt.plot(list_L, R, "xb", label="avec fonction" )
plt.legend()
plt.xlabel("Width of the rod")
plt.ylabel("r0")
plt.show(block=False)
plt.savefig("fig/r0_validation_base_Morpho_r0_fct_L_compare_creneau_with_without_functiondef.pdf")
