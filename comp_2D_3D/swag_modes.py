...
def structure(period):
    ...
    return super_pml, cube_pml, gap_pml, metal_pml, sub_pml
...
### 3D
list_number_modes = np.arange(15,31,1)
R = np.empty(list_number_modes.size, dtype = complex)
...
for i, number_modes in enumerate(list_number_modes):
    print(i)
    Mm = number_modes
    Nm = number_modes
    super_pml, cube_pml, gap_pml, metal_pml, sub_pml = structure(period)
    R[i] = reflectance_3D(super_pml, cube_pml, gap_pml, metal_pml, sub_pml)

plt.figure(3)
plt.plot(list_number_modes, R)
plt.xlabel("Number of modes")
plt.ylabel("R")
plt.title("Total reflectance of 3D structure")
plt.legend()
plt.show(block=False)
plt.savefig("3D_number_modes_impact_15_31_period400_pml.jpg")