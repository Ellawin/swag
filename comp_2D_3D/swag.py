...
def reflectance_2D(geometry, wave, materials, n_mod):  
    ...
    P1, V1 = base2D.grating(k0, a0, polarization, perm_env, perm_Ag, n, pos_reso)
    S = base2D.cascade(S, base2D.interface(Pup, P1))
    S = base2D.c_bas(S, V1, thick_reso)
    
    P2, V2 = base2D.homogene(k0, a0, polarization, perm_dielec, n)
    S = base2D.cascade(S, base2D.interface(P1, P2))
    S = base2D.c_bas(S, V2, thick_gap)

    P3, V3 = base2D.homogene(k0, a0, polarization, perm_Au, n)
    S = base2D.cascade(S, base2D.interface(P2, P3))
    S = base2D.c_bas(S, V3, thick_gold)

    P4, V4 = base2D.homogene(k0, a0, polarization, perm_Glass, n)
    S = base2D.cascade(S, base2D.interface(P3, P4))
    S = base2D.c_bas(S, V4, thick_sub)
...
R_2D = np.empty(lambdas.size, dtype = complex)

for i, lambd in enumerate(lambdas):
    print(i)
    r_3D[i] = reflectance_3D(lambd)
    ...
    R_2D[i] = reflectance_2D(geometry, wave, materials, n_mod)
...
plt.ylabel("|r|")
plt.ylim([-0.1,1.1])
plt.savefig("Cube30gold10gap3_2D_3Dcomp.png")
