...
# thick_metal = 5
# R_thermal = np.empty(list_wavelength.size)
# R_nh3 = np.empty(list_wavelength.size)
# R_n2 = np.empty(list_wavelength.size)
...
#     ri_TiN_thermal, ri_TiN_nh3, ri_TiN_n2 = nk_TiN(wavelength)
...
#     R_thermal[idx_wav] = reflectance(geometry, wave, materials_thermal, n_mod)
#     R_nh3[idx_wav] = reflectance(geometry, wave, materials_nh3, n_mod)
#     R_n2[idx_wav] = reflectance(geometry, wave, materials_n2, n_mod)


# plt.figure(1)
# plt.plot(list_wavelength, R_thermal, label = "Thermal")
# plt.plot(list_wavelength, R_nh3, label = "nh3")
# plt.plot(list_wavelength, R_n2, label = "n2")


list_wavelength = np.linspace(450,1000, 200)
list_metal = np.arange(5,56,10)
R_thermal = np.empty((list_metal.size,list_wavelength.size))
R_down = np.empty((list_metal.size,list_wavelength.size))


for idx_metal, thick_metalliclayer in enumerate(list_metal):
    print(idx_metal)
    geometry = {"thick_super": thick_super, "width_reso": width_reso, "thick_reso": thick_reso, "thick_gap": thick_gap, "thick_func": thick_func, "thick_mol": thick_mol, "thick_metalliclayer": thick_metalliclayer, "thick_sub": thick_sub, "thick_accroche": thick_accroche, "period": period}
    ...
    for idx_wav, wavelength in enumerate(list_wavelength):
        ...
        materials_n2 = {"perm_env": perm_env, "perm_dielec": perm_dielec, "perm_sub": perm_sub, "perm_reso": perm_Ag, "perm_metalliclayer": perm_Au, "perm_accroche": ri_TiN_n2 ** 2}   
        wave = {"wavelength": wavelength, "angle": angle, "polarization": polarization}
        R_thermal[idx_metal, idx_wav], R_down[idx_metal, idx_wav] = reflectance(geometry, wave, materials_thermal, n_mod)
        #R_nh3[idx_wav] = reflectance(geometry, wave, materials_nh3, n_mod)
        #R_n2[idx_wav] = reflectance(geometry, wave, materials_n2, n_mod)


plt.figure(3)
for idx_metal in np.arange(list_metal.size):
    plt.plot(list_wavelength, R_thermal[idx_metal], label = f"thickness {int(list_metal[idx_metal])} nm")

plt.legend()
plt.ylabel("Reflectance")
plt.xlabel("Wavelength (nm)")
plt.title("TiN only (R up)")

plt.show(block=False)
plt.savefig("period100_cubeAg30_gap3_mol0_func1_metalTiNVar5-56-10_envAir_RthermalUP.pdf")
plt.savefig("period100_cubeAg30_gap3_mol0_func1_metalTiNVar5-56-10_envAir_RthermalUP.jpg")

plt.figure(4)
for idx_metal in np.arange(list_metal.size):
    plt.plot(list_wavelength, R_down[idx_metal], label = f"thickness {int(list_metal[idx_metal])} nm")

plt.legend()
plt.ylabel("Reflectance")
plt.xlabel("Wavelength (nm)")
plt.title("TiN only (R down)")

plt.show(block=False)
plt.savefig("period100_cubeAg30_gap3_mol0_func1_metalTiNVar5-56-10_envAir_RthermalDOWN.pdf")
plt.savefig("period100_cubeAg30_gap3_mol0_func1_metalTiNVar5-56-10_envAir_RthermalDOWN.jpg")


np.savez("data_period100_cubeAg30_gap3_mol0_func1_metalTiNVar5-56-10_envAir_Rthermal_UP-DOWN.npz", list_wavelength = list_wavelength, R_thermal = R_thermal, R_down = R_down)
