...
plt.title("Gap layer thickness variations")
plt.xlabel("Wavelength (nm)")

plt.show(block=False)
plt.savefig("gap_1.pdf")
plt.savefig("gap_1.jpg")


file = open(f"Data_structures_gap_1.txt", 'w')
...
for i in range(len(list_wavelength)):
    file.write(f"{list_wavelength[i]}, {R_gap1_mol[i]}\t, {R_gap1_nomol[i]}\t, {R_gap2_mol[i]}\t,{R_gap2_nomol[i]}\t,{R_gap3_mol[i]}\t,{R_gap3_nomol[i]}\t,{R_gap4_mol[i]}\t,{R_gap4_nomol[i]}\t,{R_gap5_mol[i]}\t, {R_gap5_nomol[i]}\t {R_gap10_mol[i]}\t {R_gap10_nomol[i]}\n")
file.close()
