...
top_gp.eps =  np.array([[1.54, 1.54, eps_au, eps_dielec, eps_ag]])#,
top_gp.eta=eta
...
sub_sp.eps = np.array([[1.54, 1.54, eps_au, eps_dielec, eps_dielec]])
...
top_gp.ox = [0, l_pml, l_pml + l_quartz, l_pml + l_gold, l_pml + l_gold + l_gap, l_pml + l_gold + l_gap + l_cube]
top_gp.nx = [0, l_pml, l_pml + l_quartz, l_pml + l_gold, l_pml + l_gold + l_gap, l_pml + l_gold + l_gap + l_cube]
sub_sp.ox = [0, l_pml, l_pml + l_quartz, l_pml + l_gold, l_pml + l_gold + l_gap, l_pml + l_gold + l_gap + l_cube]
sub_sp.nx = [0, l_pml, l_pml + l_quartz, l_pml + l_gold, l_pml + l_gold + l_gap, l_pml + l_gold + l_gap + l_cube]

list_theta = np.linspace(0,90, 100) / 180 * np.pi
r_gp = np.empty(list_theta.size, dtype = complex)
n_gp = np.empty(list_theta.size)
...
for i, theta in enumerate(list_theta):
    a = -neff * k0 * np.sin(theta) * np.cos(phi) # inspiration Yanis # kx
    #a = -k0 * np.sin(theta) * np.cos(phi)
    top_gp.a0 = a
    sub_sp.a0 = a

    b = -neff * k0 * np.sin(theta) * np.sin(phi) #Inspiration Yanis # ky
    ...
    n_gp[i] = np.real(Vgp[index_gp] / top_gp.k0) # équivaut au kz/k0
...
plt.figure(1)
plt.plot(list_theta * 180 / np.pi, n_gp)
plt.xlabel("Theta")
#plt.legend()
plt.ylabel("$n_{GP}$")
plt.title("Effective index")
plt.show(block=False)
plt.savefig("Ngp_theta_pml40_quartz60_gold10_gap3_cube30_20modes_lam600_phi90.jpg")
...
plt.show(block=False)
plt.savefig("Rgp_theta_pml40_quartz60_gold10_gap3_cube30_20modes_lam600_phi90.jpg")

plt.figure(3)
plt.plot(list_theta * 180 / np.pi, np.angle(r_gp))
plt.xlabel("Theta")
plt.ylabel("$r_{GP}$")
plt.legend()
#plt.ylim([0,1])
...
plt.show(block=False)
plt.savefig("RgpPhi_theta_pml40_quartz60_gold10_gap3_cube30_20modes_lam600_phi90.jpg")

np.savez("data_Ngp_theta_pml40_quartz60_gold10_gap3_cube30_20modes_lam600_phi90.npz", list_theta=list_theta, r_gp = r_gp, n_gp = n_gp)
