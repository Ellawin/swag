...
r_gp = np.empty(list_pml.size, dtype = complex)
n_gp = np.empty(list_pml.size)

for i, l_pml in enumerate(list_pml):
#l_pml = 0.1
    ...
    top_gp.ox = [0, l_pml, periodx / 2 + l_pml, periodx / 2 + l_gap + l_pml, periodx + l_pml]
    top_gp.nx = [0, l_pml, periodx / 2 + l_pml, periodx / 2 + l_gap + l_pml, periodx + l_pml]
    sub_sp.ox = [0, l_pml, periodx / 2 + l_pml, periodx / 2 + l_gap + l_pml, periodx + l_pml]
    sub_sp.nx = [0, l_pml, periodx / 2 + l_pml, periodx / 2 + l_gap + l_pml, periodx + l_pml]
...
plt.figure(3)
plt.plot(list_pml, n_gp, label = 'Period (300 + pml) nm')
plt.xlabel("Taille PML")
plt.legend()
plt.ylabel("$n_{GP}$")
plt.title("Effective index")
plt.show(block=False)
plt.savefig("Ngp_PML_gap3_20modes_lam600_phi90_period300+pml.jpg")

plt.figure(4)
plt.plot(list_pml, np.abs(r_gp)**2, label = 'Period (300+pml) nm')
plt.xlabel("Taille PML")
plt.ylabel("$r_{GP}$")
plt.legend()
#plt.ylim([0,1])
...
plt.show(block=False)
plt.savefig("Rgp_PML_gap3_20modes_lam600_phi90_period300+pml.jpg")

np.savez("data_PML_gap3_20modes_lam600_phi90_period300+pml.npz", list_pml=list_pml, r_gp = r_gp, n_gp = n_gp)
