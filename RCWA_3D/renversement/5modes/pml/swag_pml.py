...
plt.figure(1)
plt.plot(list_pml, n_gp, label = 'Period 300 nm')
plt.xlabel("Taille PML")
plt.legend()
plt.ylabel("$n_{GP}$")
plt.title("Effective index")
plt.show(block=False)
plt.savefig("Ngp_PML_gap3_5modes_lam600_period300_phi90_period300.jpg")

plt.figure(2)
plt.plot(list_pml, np.abs(r_gp)**2, label = 'Period 300 nm')
plt.xlabel("Taille PML")
plt.ylabel("$r_{GP}$")
plt.legend()
#plt.ylim([0,1])
...
plt.show(block=False)
plt.savefig("Rgp_PML_gap3_5modes_lam600_period300_phi90_period300.jpg")

np.savez("data_Ngp_PML_gap3_5modes_lam600_period300_phi90_period300.npz", list_pml=list_pml, r_gp = r_gp, n_gp = n_gp)
