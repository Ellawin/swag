...
list_theta = np.linspace(0,90,500) / 180 * np.pi
...
for i, theta in enumerate(list_theta):
    #a = -neff * k0 * np.sin(theta) * np.cos(phi) # inspiration Yanis
    a = -k0 * np.sin(theta) * np.cos(phi)
    top_gp.a0 = a
    sub_sp.a0 = a

    #b = -neff * k0 * np.sin(theta) * np.sin(phi) #Inspiration Yanis
...
plt.figure(1)
plt.plot(list_theta * 180 / np.pi, n_gp, label = 'gap 10 nm')
plt.xlabel("Theta")
plt.legend()
plt.ylabel("$n_{GP}$")
plt.title("Effective index")
plt.show(block=False)
plt.savefig("Ngp_theta_variable_renversed_system_gap10_5modes_lam600_period300_without_neff_phi90_pml40.jpg")

plt.figure(2)
plt.plot(list_theta * 180 / np.pi, np.abs(r_gp) ** 2, label = 'gap 10 nm')
plt.xlabel("Theta")
plt.ylabel("$r_{GP}$")
plt.legend()
#plt.ylim([0,1])
...
plt.show(block=False)
plt.savefig("Rgp_theta_variable_renversed_system_gap10_5modes_lam600_period300_without_neff_phi90_pml40.jpg")
