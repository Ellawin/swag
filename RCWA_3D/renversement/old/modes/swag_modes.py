...
plt.figure(3)
plt.plot(list_Mm, n_gp, label = 'PML 40 nm, period 300 nm')
plt.xlabel("Nombre de modes")
plt.legend()
plt.ylabel("$n_{GP}$")
plt.title("Effective index")
plt.show(block=False)
plt.savefig("Ngp_modes30values_gap3_lam600_phi90_pml40_period300.jpg")

plt.figure(4)
plt.plot(list_Mm, np.abs(r_gp)**2, label = 'PML 40 nm, période 300 nm')
plt.xlabel("Nombre de modes")
plt.ylabel("$r_{GP}$")
plt.legend()
#plt.ylim([0,1])
...
plt.show(block=False)
plt.savefig("Rgp_modes30values_gap3_lam600_phi90_pml40_period300.jpg")

np.savez("data_Ngp_modes30values_gap3_lam600_phi90_pml40_period300.npz", list_Mm=list_Mm, r_gp = r_gp, n_gp = n_gp)
