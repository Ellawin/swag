...
GP_effective_index = modes.steepest(start_index_eff, tol, step_max, Layers, wavelength, polarization)
#idx_gap+=1

print("start = ", start_index_eff)
print("mode found = ", np.real(GP_effective_index))
...
# plt.savefig(f"data/nGP_PM_Cube-Thick{thick_cube}-MatAg_Gap-Thick-Var-1-15-100-Mat1.45_Metal-Thick{thick_metal}-MatAu_Sub-Thick{thick_sio2}-MatSiO2.pdf")

# #np.savez(f"data/nGP_PM_Cube-Thick{thick_cube}-MatAg_Gap-Thick-Var-1-15-100-Mat1.45_Metal-Thick{thick_metal}-MatAu_Sub-Thick{thick_sio2}-MatSiO2.pdf", ngp =GP_effective_index, list_gap = list_gap)
# import datetime
# now = datetime.datetime.now().strftime("%y_%m_%d_%X")
# file = open(f"data/{now}_nGP_PM_Cube-Thick{thick_cube}-MatAg_Gap-Thick-Var-1-15-100-Mat1.45_Metal-Thick{thick_metal}-MatAu_Sub-Thick{thick_sio2}-MatSiO2.pdf", 'w')
# for i in range(len(list_gap)):
#     file.write(f"{list_gap[i]}, {GP_effective_index[i]}, {start_index_eff[i]}\n")
# file.close()