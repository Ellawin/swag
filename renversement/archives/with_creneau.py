...
def reflectance(thick_up, thick_down, thick_gap, thick_reso, thick_gold, period, wavelength, angle, polarization, perm_dielec, perm_Ag, n_mod):    
    ...
    Pup, Vup = creneau(k0, a0, polarization, perm_Ag, 1, thick_reso, n, 0)
    Pdown, Vdown = creneau(k0, a0, polarization, 1, perm_Ag, thick_gap, n, thick_reso)
   ...
   # reflexion quand on eclaire par le dessus
    ...
    Tup = abs(S[n + position_down, position_up]) ** 2 * np.real(Vdown[position_up]) / (k0 * np.cos(angle)) / perm_dielec
    ...
    Tdown = abs(S[position_up, n + position_down]) ** 2 / np.real(Vdown[position_down]) * perm_dielec * k0 * np.cos(angle)
    ...
    return Rup, Tup, Rdown, Tdown, phase_R_down, phase_R_up 
...
# plt.colorbar()
# plt.title("Renversement, air et argent")
# plt.show(block=False)
# plt.savefig("Champs_renversement_air_argent_lam400.jpg") 
...
for wavelength in list_wavelength:
     ...
     R_up_NR[idx], T_up_NR[idx], R_down_NR[idx], T_down_NR[idx], phase_R_down_NR[idx], phase_R_up_NR[idx] = reflectance(thick_up, thick_down, thick_gap, thick_reso, thick_gold, period, wavelength, angle, polarization, 1, perm_Ag, n_mod)
     idx += 1
...
plt.show(block=False)
plt.savefig("Phase_and_modulus_reflexion_GP_depending_wavelength_creneauVersion.jpg")
...
plt.title("Reflectance of the GP")
plt.show(block=False)
plt.savefig("Modulus_zoom__reflexion_GP_depending_wavelength.jpg")
