...
### SWAG functions
def reflectance(thick_up, thick_down, width_reso, thick_gap, thick_reso, thick_gold, period, wavelength, angle, polarization, perm_dielec, perm_Ag, n_mod):    
    n = 2 * n_mod + 1
    ...
    Pin, Vin = homogene(k0, a0, polarization, 1, n)
    Pup, Vup = reseau(k0, a0, polarization, 1, perm_Ag, n, blocs_1)
    S = cascade(S, interface(Pin, Pup))
    S = c_bas(S, Vup, width_reso)
    Pdown, Vdown = reseau(k0, a0, polarization, perm_dielec, perm_Ag, n, blocs_2)
    S = cascade(S, interface(Pup, Pdown))
    S = c_bas(S, Vdown, width_reso)
    Pout, Vout = homogene(k0, a0, polarization, 1, n)
    S = cascade(S, interface(Pdown, Pout))
    S = c_bas(S, Vout, thick_down)

    #Éclairage par au dessus, onde plane
    Rup = abs(S[n_mod, n_mod]) ** 2 #* np.real(Vup[position_up]) / (1 * k0 * np.cos(angle))
    Tup = abs(S[n + n_mod, n_mod]) ** 2 #* np.real(Vout[n_mod]) / (k0 * np.cos(angle)) / perm_dielec
    #Éclairage par en dessous, onde plane
    Rdown = abs(S[n + n_mod, n + n_mod]) ** 2 #* np.real(Vdown[position_down]) / (np.sqrt(perm_dielec) * k0 * np.cos(angle))
    Tdown = abs(S[n_mod, n + n_mod]) ** 2 # / np.real(Vdown[position_down]) * perm_dielec * k0 * np.cos(angle)

    # Éclairage par en dessous, guide d'onde
    #position_down = np.argmax(np.real(Vdown))
    #print("position_down = %f" %position_down)

    # Éclairage par au dessus, guide d'onde
    #position_up = np.argmax(np.real(Vup))
    #print("position_up = %f" %position_up)

   # reflexion quand on eclaire par le dessus
    #Rup = abs(S[position_up, position_up]) ** 2 * np.real(Vup[position_up]) / (1 * k0 * np.cos(angle))
    # transmission quand on éclaire par le dessus
    #Tup = abs(S[n + position_up, position_up]) ** 2 * np.real(Vdown[position_up]) / (k0 * np.cos(angle)) / perm_dielec
    # reflexion quand on éclaire par le dessous
    #Rdown = abs(S[n + position_down, n + position_down]) ** 2 * np.real(Vdown[position_down]) / (np.sqrt(perm_dielec) * k0 * np.cos(angle))
    # transmission quand on éclaire par le dessous
    #Tdown = abs(S[position_down, n + position_down]) ** 2 / np.real(Vdown[position_down]) * perm_dielec * k0 * np.cos(angle)
...
def Field_up(thick_up, thick_down, width_reso, thick_gap, thick_reso, thick_gold, period, wavelength, angle, polarization, perm_dielec, perm_Ag, n_mod):
    wavelength_norm = wavelength / period
    ...
    Pin, Vin = homogene(k0, a0, polarization, 1, n_mod_total)
    A.append([Pin.tolist(), Vin.tolist()])     

    # milieu incident, metal d'argent puis air
    Pup, Vup = reseau(k0, a0, polarization, 1, perm_Ag, n_mod_total, blocs_1)
    A.append([Pup.tolist(), Vup.tolist()])     

    # couche 2 : cube d'argent dans couche d'air
    Pdown, Vdown = reseau(k0, a0, polarization, perm_dielec, perm_Ag, n_mod_total, blocs_2)
    A.append([Pdown.tolist(), Vdown.tolist()])

    Pout, Vout = homogene(k0, a0, polarization, 1, n_mod_total)
    A.append([Pout.tolist(), Vout.tolist()])

    thickness = np.array([thick_up, width_reso, width_reso, thick_down])
...
def Field_down(thick_up, thick_down, width_reso, thick_gap, thick_reso, thick_gold, period, wavelength, angle, polarization, perm_dielec, perm_Ag, n_mod):
    wavelength_norm = wavelength / period
...
# wavelength = 400
# perm_Ag = epsAgbb(wavelength) # argent
# Mfieldup = Field_up(thick_up, thick_down, width_reso, thick_gap, thick_reso, thick_gold, period, wavelength, angle, polarization, perm_dielec, perm_Ag, n_mod)
# Mfield_down = Field_down(thick_up, thick_down, width_reso, thick_gap, thick_reso, thick_gold, period, wavelength, angle, polarization, perm_dielec, perm_Ag, n_mod)

# plt.figure(4)
# plt.imshow(Mfieldup, cmap = 'jet', aspect = 'auto')
# plt.colorbar()
# plt.title("Up")
# plt.show(block=False)
# plt.savefig("Champs_renversement_homogene_up_75_theta20.jpg") 

# plt.figure(5)
# plt.imshow(Mfield_down, cmap = 'jet', aspect = 'auto')
# plt.title("Down")
# plt.show(block=False)
# plt.savefig("Champs_renversement_homogene_down_75_theta20.jpg") 


## Pour étudier l'influence de la longueur d'onde sur le coefficient de réflexion du GP
list_wavelength = np.linspace(200, 800, 100)

R_up_NR_theta0 = np.empty(list_wavelength.size)
R_down_NR_theta0 = np.empty(list_wavelength.size)
R_up_NR_theta20 = np.empty(list_wavelength.size)
R_down_NR_theta20 = np.empty(list_wavelength.size)
...
for wavelength in list_wavelength:
     ...
     R_up_NR_theta0[idx], T_up_NR[idx], R_down_NR_theta0[idx], T_down_NR[idx], phase_R_down_NR[idx], phase_R_up_NR[idx] = reflectance(thick_up, thick_down, width_reso, thick_gap, thick_reso, thick_gold, period, wavelength, angle, polarization, perm_dielec, perm_Ag, n_mod)
     R_up_NR_theta20[idx], T_up_NR[idx], R_down_NR_theta20[idx], T_down_NR[idx], phase_R_down_NR[idx], phase_R_up_NR[idx] = reflectance(thick_up, thick_down, width_reso, thick_gap, thick_reso, thick_gold, period, wavelength, angle, polarization, perm_dielec, perm_Ag, n_mod)
     idx += 1

plt.figure(7)
#plt.subplot(211)
plt.plot(list_wavelength, R_up_NR_theta0, "r", label="theta 0")
#plt.plot(list_wavelength, R_down_NR_theta0, "b", label="R down NR")
plt.plot(list_wavelength, R_up_NR_theta20, "b", label="theta 20")
#plt.plot(list_wavelength, R_down_NR_theta0, "b", label="R down NR")
...
plt.show(block=False)
plt.savefig("Phase_and_modulus_reflexion_GP_depending_wavelength_theta0_theta20.jpg")
...
# plt.show(block=False)
# plt.savefig("Phase_and_modulus_reflexion_GP_depending_wavelength_homogene_75_theta20.jpg")
