...
### SWAG functions
def reflectance(thick_up, thick_down, thick_gap, thick_reso, thick_gold, period, wavelength, angle, polarization, perm_dielec, perm_Ag, n_mod):    
    n = 2 * n_mod + 1
    ...
    #S = c_bas(S, Vdown, thick_down) # ligne utile seulement pour visualiser le champ 
    ...
    ## Version 1 (moins fiable qu'en utilisant PM)

    #position_up = np.argmax(np.real(Vup)) 
    #print("position up = ", position_up)
...
def Field(thick_up, thick_down, thick_gap, thick_reso, thick_gold, period, wavelength, angle, polarization, perm_dielec, perm_Ag, n_mod):
    ...
    Pup, Vup = creneau(k0, a0, polarization, perm_Ag, perm_dielec, thick_reso, n_mod_total, 0)
    A.append([Pup.tolist(), Vup.tolist()])     

    # couche 2 : cube d'argent dans couche d'air
    Pdown, Vdown = creneau(k0, a0, polarization, perm_dielec, perm_Ag, thick_gap, n_mod_total, thick_reso)
...
list_period = np.linspace(100, 1500, 100)
R_down_GP = np.empty(list_period.size)
phase_R_down_GP = np.empty(list_period.size)

idx = 0

for period in list_period:
     R_down_GP[idx], phase_R_down_GP[idx] = reflectance(thick_up, thick_down, thick_gap, thick_reso, thick_gold, period, wavelength, angle, polarization, 1, perm_Ag, n_mod)
     idx += 1

plt.figure(2)
plt.subplot(211)
plt.plot(list_period, R_down_GP, "g", label="R down GP")
plt.legend()
plt.ylabel("Module")
...
plt.subplot(212)
plt.plot(list_period, phase_R_down_GP, "g", label="phase R down GP")
plt.legend()
plt.xlabel("Period (nm) ")
plt.ylabel("Phase")
#plt.title("Wavelength dependance")
plt.show(block=False)
plt.savefig("Phase_and_modulus_reflexion_GP_depending_period.jpg")

plt.figure(3)
plt.plot(list_period, R_down_GP, "b", label="R down GP")
plt.ylabel("Module")
plt.title("Reflectance of the GP")
plt.show(block=False)
plt.savefig("Modulus_zoom__reflexion_GP_depending_period.jpg")

"""
neff_min = 1.000
neff_max = 1.3
modes_GP = pm.guided_modes(Layer_down, wavelength,polarization, neff_min, neff_max)
print(modes_GP)

Guided_modes will fail here because its limited number of steps before giving up does not allow it to reach the effective index of the mode.
Hence the direct use of the steepest descent with adapted parameters.
"""
...
# Mfield2 = Field(thick_up, thick_down, thick_gap, thick_reso, thick_gold, period, wavelength, angle, polarization, 1, perm_Ag, n_mod)
# plt.figure(7)
# plt.imshow(Mfield2, cmap = 'jet', aspect = 'auto')
# plt.colorbar()
# plt.title("Renversement, air et argent, wavelength 300 nm")
# plt.show(block=False)
# plt.savefig("Champs_renversement_air_argent_lam300_75modes.jpg") 
...
# plt.figure(2)
# plt.subplot(211)
# plt.plot(list_wavelength, R_down_GP, "g", label="R down GP")
# plt.legend()
# plt.ylabel("Module")
# plt.title("Reflectance of the GP")
# plt.show(block=False)
...
# plt.subplot(212)
# plt.plot(list_wavelength, phase_R_down_GP, "g", label="phase R down GP")
# plt.legend()
# plt.xlabel("Wavelength (nm) ")
# plt.ylabel("Phase")
# #plt.title("Wavelength dependance")
# plt.show(block=False)
# plt.savefig("Phase_and_modulus_reflexion_GP_depending_wavelength_creneauVersion_GP.jpg")

# plt.figure(3)
# plt.plot(list_wavelength, R_down_GP, "b", label="R down NR")
# plt.ylabel("Module")
# plt.title("Reflectance of the GP")
# plt.show(block=False)
# plt.savefig("Modulus_zoom__reflexion_GP_depending_wavelength_GP.jpg")
